...
- `arguments[]`不会反应参数的默认值，只反映传给函数的默认值。即：即使你定义了默认值，`arguments[]`依然不会返回默认值，只会返回你传入的参数

#### 暂时性死区：前面定义的参数不能引用后面定义的，也不能引用函数体的作用域。

### 10.6 参与扩展与收集

### 10.7-10.8  ES6语法 暂不查看

### 10.9 函数内部：`arguments`&`this`

`arguments`:类数组对象，包含调用函数时传入的所有参数
