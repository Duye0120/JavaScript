...
noscript:在脚本不可用时让浏览器显式一段话，如果浏览器支持脚本，则用户永远不会看到这句话。
...
符号是原始值，且符号实例是**唯一、不可变的**，不能和其他数据进行运算

用途：确保对象属性使用唯一标识符，不会发生属性属性冲突的危险

### 1.符号的基本用法

符号需要使用`Symbol()`进行初始化。

调用`Symbol()`函数时，可以传入一个字符串参数用作藐视，未来将通过这个字符串调试代码。注：这个字符串参数与符号定义或标识完全无关

**Symbol函数不能用作构造函数，与new关键字一起使用**

### 2.使用全局符号注册表

如果运行时的不同部分需要共享和重用符号实例，那么可以用一个字符串作为键，在全局符号注册表中创建并重用符号。

使用`Symbol.for()`方法：对每个字符串都执行幂等操作.

全局注册表中的符号必须使用字符串来创建。因此，注册表中使用的键同时也会被用作符号描述。
...
引入一批**常用内置符号**，用于暴露语言内部行为，开发者可以直接访问、重写或模拟这些行为。
